  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (result?.data) setProfile(result.data as ViewableProfile);
      } catch (err) {
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-sm text-center">
        ...
        <div className="flex flex-col items-center gap-4 py-4">
          {profile?.photo1Url ? (
            <img src={profile.photo1Url} alt="マッチした相手"
              className="h-32 w-32 rounded-full object-cover border-4 border-primary" />
          ) : (
            <div className="h-32 w-32 rounded-full bg-muted flex items-center justify-center text-4xl">💑</div>
          )}
          {profile?.displayName && (
